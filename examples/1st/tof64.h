...
static uint16_t tof64[TOF64_DTMAX];

void setupTof64(void) {
  ...
  memset(tof64, 0x0, sizeof(tof64));
}

void dumpTof64(int st, int en) {
  for (int y = st; y < en; y++) {
    int pos = (y * tof64Width);
    for (int x = 0; x < tof64Width; x++) {
      Serial.printf("%6d ", tof64[pos + x]);
    }
    Serial.println();
  }
  Serial.println();
    ...
    if (tof64Img.getRangingData(&tof64Data)) {
      ...
      for (int y = 0 ; y <= tof64Width * (tof64Width - 1) ; y += tof64Width) {
        for (int x = tof64Width - 1 ; x >= 0 ; x--) {
          tof64[pos] = tof64Data.distance_mm[x + y];
          pos++;
        }
      }
    }
